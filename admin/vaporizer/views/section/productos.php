...
?>
   ...
   $items = $webDB->order_by('id', 'DESC')->where("pub_status = 'published' AND vap_itemstatus != 'empty'")->get('productos',$limit,$offset)->result();
    ?>

<div id="secTopBar">
    <div class="rute">Productos (publicados)</div>
    <div class="tools"><div id="pager"><a href="<?=base_url('productos/'.($extra-1))?>"><</a> <?=$desde?>-<?=$hasta?> de <?=$noitems?>  <a href="<?=base_url('productos/'.($extra+1))?>">></a></div><button class="topButton principal" onclick="newItem();">+ Nuevo</button></div>
</div>
        ...
        <tbody> 
            ...
            foreach ($items as $item):
                ...
                <tr>
                    ...
                    <td style="padding: 0px 10px;"><?= $item->nombre ?> <button class="archBtn" data-id="<?= $item->id ?>">ARCHIVAR</button> <button class="editBtn" data-id="<?= $item->id ?>">[EDIT]</button></td>
