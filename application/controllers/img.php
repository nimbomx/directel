...
//v1.0
    ...
    function thumb($ancho, $alto, $dir, $url) {
        ...
        $parts = pathinfo($file);
        //$file=$parts ['basename'];
        $filethumb='t_'.$ancho.'x'.$alto.'_'.$parts ['basename'];
        //echo $file;return false;
        if(file_exists ( './res/'.$dir.'/'.$filethumb )){
            header('Content-Type: image/jpeg');
            readfile('./res/'.$dir.'/'.$filethumb);
            return false;
        }
        $quality = 90;
        ...
        $salida=    FCPATH . './res/' . $dir . '/' . ($filethumb);
        ...
        $imagen = @ImageCreateFromJPEG($file) or // Read JPEG Image
                ...
                $imagen = false;
        $noblack = 1;
        if (isset($_POST['noblack']))
            $noblack = $_POST['noblack'];
        ...
        switch ($type) {
            case 1: imagegif($imagen_p, $salida);
                break;
            case 2: imagejpeg($imagen_p, $salida, $quality);
                break;
            case 3: imagepng($imagen_p, $salida);
                break;
        ...
        header("Content-type: image/jpeg");
        switch ($type) {
            case 1: imagegif($imagen_p, NULL);
                break;
            case 2: imagejpeg($imagen_p, NULL, $quality);
                break;
            case 3: imagepng($imagen_p, NULL);
                break;
            default: trigger_error('Failed resize image!', E_USER_WARNING);
                break;
        }
